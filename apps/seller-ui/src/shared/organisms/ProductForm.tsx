...
type ProductFormMode = 'product' | 'event';
...
const ProductForm: React.FC<ProductFormProps> = ({ mode, title, breadcrumbLabel, onSuccessPath }) => {
  ...
  const onSubmit = (formData: any) => {
    ...
    if (mode === 'event') {
      payload.starting_date = formData.starting_date;
      payload.ending_date = formData.ending_date;
    }
  ...
  return (
    <form className="w-full mx-auto p-8 shadow-md rounded-lg" onSubmit={handleSubmit(onSubmit)}>
      <h2 className="text-2xl py-2 font-semibold font-Poppins">{title}</h2>
      <div className="flex items-center">
        <span className="cursor-pointer opacity-90">Dashboard</span>
        <ChevronRight />
        <span className="cursor-pointer">{breadcrumbLabel}</span>
      </div>
            ...
            <div className="w-2/4">
              ...
              <div className="mt-2">
                ...
                {errors.cash_on_delivery && (
                  <p className="text-red-500">{(errors.cash_on_delivery.message as string) || ''}</p>
                )}
              </div>
            </div>
            ...
            <div className="w-2/4">
                ...
                <Controller
                  ...
                  rules={{
                    ...
                    validate: (value) => {
                      ...
                      if (wordCount < 100) return 'Detailed description must be at least 100 words';
                      return true;
                    },
                  }}
                  render={({ field }) => (
              ...
              <div className="mt-2">
                ...
                {errors.regular_price && (
                  <p className="text-red-500">{(errors.regular_price.message as string) || ''}</p>
                )}
              </div>
              ...
              <div className="mt-2">
                ...
                <Input
                  ...
                  {...register('stock', {
                    ...
                    max: { value: 1000000, message: 'Stock must be less than 1,000,000' },
                    validate: (value) => {
              ...
              {mode === 'event' && (
                <div className="mt-2 space-y-3">
                  <div>
                    ...
                    <input
                      type="datetime-local"
                      className="w-full border border-border rounded-md p-2 text-foreground bg-background"
                      {...register('starting_date', {
                        required: 'Start date is required',
                      })}
                    />
                    {errors.starting_date && (
                      <p className="text-red-500">{(errors.starting_date.message as string) || ''}</p>
                    )}
                  </div>
                  <div>
                    ...
                    />
                    {errors.ending_date && (
                      <p className="text-red-500">{(errors.ending_date.message as string) || ''}</p>
                    )}
                  </div>
                </div>
              )}
      ...
      <div className="mt-6 flex justify-end gap-3">
        {isChanged && (
          <button className="px-4 py-2 rounded-md bg-secondary text-secondary-foreground" type="button">
            Save Draft
          </button>
        )}
        <button className="px-4 py-2 bg-primary rounded-md" type="submit">
          {mode === 'event' ? 'Publish Event' : 'Publish'}
        </button>
      </div>
    </form>
  );
